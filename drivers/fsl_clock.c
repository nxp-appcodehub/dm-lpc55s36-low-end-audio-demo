...
#define GET_HFXO_TRIM()  (*(uint32_t *)(FLASH_NMPA_BASE + 0xC0U)) // (0x3FCC0)
#define GET_32KXO_TRIM() (*(uint32_t *)(FLASH_NMPA_BASE + 0xC4U)) // (0x3FCC4)
    ...
    if (kNONE_to_NONE != connection)
    {
        for (i = 0U; i < 2U; i++)
        {
            ...
            if (item != 0U)
            {
                ...
                {
                    PMC->OSEVENTTIMER =
                        (PMC->OSEVENTTIMER & ~PMC_OSEVENTTIMER_SELCLOCK_MASK) | PMC_OSEVENTTIMER_SELCLOCK(sel);
                }
                else
                {
...
clock_attach_id_t CLOCK_GetClockAttachId(clock_attach_id_t attachId)
{
    ...
    for (i = 0U; i < 2U; i++)
    {
        ...
        if (tmp32 != 0UL)
        {
            ...
            {
                actualSel = ((PMC->OSEVENTTIMER) & PMC_OSEVENTTIMER_SELCLOCK_MASK) >> PMC_OSEVENTTIMER_SELCLOCK_SHIFT;
            }
            else
            {
...
void CLOCK_SetClkDiv(clock_div_name_t div_name, uint32_t divided_by_value, bool reset)
{
    ...
    else
    {
        if (reset)
        {
            ((volatile uint32_t *)pClkDiv)[(uint16_t)div_name] = 1UL << 29U;
        }
        if (divided_by_value == 0U) /*!<  halt */
        {
            ((volatile uint32_t *)pClkDiv)[(uint16_t)div_name] = 1UL << 30U;
        }
        else
        {
            ((volatile uint32_t *)pClkDiv)[(uint16_t)div_name] = (divided_by_value - 1U);
        }
    }
}
...
status_t CLOCK_SetupFROClocking(uint32_t iFreq)
{
    ...
    SYSCON->PRESETCTRLCLR[2] = SYSCON_PRESETCTRL2_ANACTRL_RST_MASK;
...
void CLOCK_SetFLASHAccessCyclesForFreq(uint32_t system_freq_hz)
{
    ...
    while ((FLASH->INTSTAT & FLASH_INTSTAT_DONE_MASK) == 0UL)
    {
    }
...
status_t CLOCK_SetupExtClocking(uint32_t iFreq)
{
    ...
    POWER_DisablePD(kPDRUNCFG_PD_XTALHF);
...
/* Get Flexcomm 32K Clk */
/*! brief  Return Frequency of Flexcomm 32kHz
 *  return Frequency of Flexcomm 32kHz
 */
...
uint32_t CLOCK_GetPllClkDivFreq(void)
{
    ...
    {
        case 0U:
            freq = CLOCK_GetPll0OutFreq() / ((SYSCON->PLLCLKDIV & SYSCON_PLLCLKDIV_DIV_MASK) + 1U);
            break;
        case 1U:
            freq = CLOCK_GetPll1OutFreq() /
                   ((SYSCON->PLLCLKDIV & SYSCON_PLLCLKDIV_DIV_MASK) + 1U); // register name should be checked
            break;
        default:
            freq = 0U;
            break;
    }
    /* todo: pll_clk_div = pll_clk/div*/
    return freq;
}
...
uint32_t CLOCK_GetHsLspiClkFreq(void)
{
    ...
    switch (SYSCON->HSSPICLKSEL)
    {
        case 0U:
            freq = CLOCK_GetCoreSysClkFreq();
            break;
        case 1U:
            freq = CLOCK_GetPll0OutFreq() / ((SYSCON->PLLCLKDIV & 0xffU) + 1U);
            break;
        case 2U:
            freq = CLOCK_GetFro12MFreq();
            break;
...
/* Get DMIC Clk */
/*! brief  Return Frequency of DMIC clock
...
uint32_t CLOCK_GetI3cClkFreq(void)
{
    ...
    return freq / ((SYSCON->I3CFCLKDIV & SYSCON_I3CFCLKDIV_DIV_MASK) + 1U);
}
...
uint32_t CLOCK_GetI3cSTCClkFreq(void)
{
    ...
    return freq / ((SYSCON->I3CFCLKSTCDIV & SYSCON_I3CFCLKSTCDIV_DIV_MASK) + 1U);
}
...
/* Enables and sets LDO for High Frequency crystal oscillator. */
/*! brief Enables and sets LDO for High Frequency crystal oscillator.
 * return   none
...
void CLOCK_SetXtalHfLdo(void)
{
    ...
    /* Enable LDO XO32M */
    PMC->PDRUNCFGCLR0 = PMC_PDRUNCFG0_PDEN_LDOXTALHF_MASK;
}
